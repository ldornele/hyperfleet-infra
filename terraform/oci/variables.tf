...
variable "postgresql_shape" {
  description = "Compute shape for the db system's instance node. See architecture ADR 0022 for why PostgreSQL.VM.Standard.E5.Flex was chosen."
  type        = string
...
variable "postgresql_admin_password_secret_id" {
  ...
  validation {
    condition     = !var.postgresql_enabled || try(trimspace(var.postgresql_admin_password_secret_id), "") != ""
    error_message = "postgresql_admin_password_secret_id is required and cannot be empty or whitespace-only when postgresql_enabled is true."
  }
}

variable "postgresql_admin_password_secret_version" {
  description = <<-EOT
    Version of the Vault secret specified in postgresql_admin_password_secret_id.
    No default: required once postgresql_enabled is true — the OCI provider
    requires this when password_type = VAULT_SECRET.
  EOT
  type        = string
  default     = null

  validation {
    condition     = !var.postgresql_enabled || try(trimspace(var.postgresql_admin_password_secret_version), "") != ""
    error_message = "postgresql_admin_password_secret_version is required and cannot be empty or whitespace-only when postgresql_enabled is true."
  }
}
