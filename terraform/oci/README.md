...
`postgresql_subnet_id`, `postgresql_admin_password_secret_id` (a Vault secret
OCID — plaintext admin passwords aren't supported), and
`postgresql_admin_password_secret_version` are also required once enabled;
variable `validation` blocks enforce those three.

The defaults for shape (`PostgreSQL.VM.Standard.E5.Flex`) and PostgreSQL version (`17`)
